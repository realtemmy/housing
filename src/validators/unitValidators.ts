...
export const unitValidator = z.object({
  ...
  sqft: z.number().int().positive("Square footage must be a positive integer").optional(),
  status: z.enum(["AVAILABLE", "OCCUPIED", "MAINTENANCE", "RESERVED"]).optional(),
  rentAmount: z.number().positive("Rent amount must be a positive number"),
  depositAmount: z.number().nonnegative("Deposit amount must be a non-negative number").optional(),
